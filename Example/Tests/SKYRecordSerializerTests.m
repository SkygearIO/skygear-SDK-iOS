...
describe(@"serialize", ^{
    ...
    beforeEach(^{
        ...
        record = [[SKYRecord alloc] initWithRecordID:[[SKYRecordID alloc] initWithRecordType:@"book" name:@"book1"] data:nil];
    ...
    it(@"serialize asset", ^{
        SKYAsset *asset = [SKYAsset assetWithName:@"asset-name" url:[NSURL URLWithString:@"http://ourd.test/files/asset-name"]];
    ...
    it(@"serialize reference", ^{
        [record setObject:[[SKYReference alloc] initWithRecordID:[[SKYRecordID alloc] initWithRecordType:@"author" name:@"author1"]]
                   forKey:@"author"];
        NSDictionary *dictionary = [serializer dictionaryWithRecord:record];
    ...
    it(@"serialize access control", ^{
        SKYAccessControlEntry *entry = [SKYAccessControlEntry writeEntryForRelation:[SKYRelation relationFollow]];
