...
describe(@"serialize predicate", ^{
    ...
    it(@"equal ref", ^{
        SKYReference *ref = [[SKYReference alloc] initWithRecordID:[[SKYRecordID alloc] initWithRecordType:@"city" name:@"hongkong"]];
        NSArray *result = [serializer serializeWithPredicate:
    ...
    it(@"equal ref to user record id", ^{
        SKYReference *ref = [[SKYReference alloc] initWithRecordID:[[SKYUserRecordID alloc] initWithUsername:@"john.doe@example.com"]];
        NSArray *result = [serializer serializeWithPredicate:
                           [NSPredicate predicateWithFormat:@"_created_by = %@", ref]];
        expect([result class]).to.beSubclassOf([NSArray class]);
        expect(result[0]).to.equal(@"eq");
