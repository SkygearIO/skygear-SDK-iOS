...
describe(@"relation add", ^{
    ...
    it(@"multiple relations", ^{
        SKYAddRelationsOperation *operation = [SKYAddRelationsOperation operationWithType:@"follow"
        ...
        waitUntil(^(DoneCallback done) {
            operation.addRelationsCompletionBlock = ^(NSArray *savedUsers, NSError *operationError) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    expect(savedUsers[0]).to.equal([SKYUserRecordID recordIDWithUsername:@"user1001"]);
