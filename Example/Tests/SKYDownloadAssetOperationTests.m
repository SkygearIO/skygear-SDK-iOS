...
describe(@"upload asset", ^{
    ...
    beforeEach(^{
        ...
        asset = [SKYAsset assetWithName:@"prefixed-boy.txt" data:[[NSData alloc] initWithBase64EncodedString:BASE64_ENCODED_CONTENT options:0]];
    ...
    it(@"downloads remote file with completion", ^{
        ...
        waitUntil(^(DoneCallback done) {
            operation.downloadAssetCompletionBlock = ^(SKYAsset *returningAsset, NSData *data, NSError *operationError) {
                expect(returningAsset).to.beIdenticalTo(asset);
