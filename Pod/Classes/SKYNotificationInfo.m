...
- (NSUInteger)hash
{
    return self.alertActionLocalizationKey.hash ^ self.alertBody.hash ^ self.alertLaunchImage.hash ^
           ...
           self.shouldBadge ^ self.shouldSendContentAvailable ^ self.desiredKeys.hash;
}
...
- (NSString *)description
{
    return [NSString stringWithFormat:@"%@ <alertBody = %@, alertLocalizationKey = %@, "
                                      ...
                                      @"%@, shouldSendContentAvailable = %@, desiredKeys = %@>",
                                      ...
                                      @(self.shouldSendContentAvailable), self.desiredKeys];
}
