...
/**
 <SKYFetchRecordsOperation> is a subclass of <SKYOperation> that implements fetching records from Ourd. Use this to
 ...
 with all the fetched records, or an error will be returned stating the error. For each <SKYRecordID>, the <perRecordCompletionBlock>
...
/**
 Sets or returns a block to be called for progress information for each <SKYRecordID>. This is only called
...
/**
 Sets or returns a block to be called when a record fetch operation completes for a <SKYRecordID>. If
 ...
 */
@property(nonatomic, copy) void (^perRecordCompletionBlock)(SKYRecord *record, SKYRecordID *recordID, NSError *error);
