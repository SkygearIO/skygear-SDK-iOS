...
- (void)recordStorageFetchUpdates:(SKYRecordStorage *)storage completionHandler:(void(^)(BOOL finished, NSError *error))completionHandler
{
    ...
    op.queryRecordsCompletionBlock = ^(NSArray *fetchedRecords, SKYQueryCursor *cursor,
    ...
    completionHandler:(void (^)(BOOL finished, NSError *error))completionHandler;
{
    ...
    [changes enumerateObjectsUsingBlock:^(SKYRecordChange *change, NSUInteger idx, BOOL *stop) {
        if (change.action == SKYRecordChangeSave) {
            SKYRecord *recordToSave = [self _constructRecordForSavingWithStorage:storage
                                                                         change:change];
            
            SKYModifyRecordsOperation *op = [[SKYModifyRecordsOperation alloc]
            ...
            SKYDeleteRecordsOperation *op = [[SKYDeleteRecordsOperation alloc]
